...
void LCD_SetRotation(uint8_t angle)
{
  LCD_WriteCommand(0x36);  // MADCTL register

  switch (angle) {
    case 0:
      LCD_WriteData(0x00);  // Normal
      break;
    case 90:
      LCD_WriteData(0x60);  // MX + MV
      break;
    case 180:
      LCD_WriteData(0xC0);  // MX + MY
      break;
    case 270:
      LCD_WriteData(0xA0);  // MY + MV
      break;
    default:
      Serial.println("Invalid rotation angle. Use 0, 90, 180, or 270.");
      break;
  }
}
